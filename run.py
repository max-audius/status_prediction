...
with open("/app/data_path.json") as fr:
    dc = json.load(fr)
with open("/app/hyperparams.json") as fr:
    hc = json.load(fr)
...
# model_path = "/app/best_model.pt"  # Correct path inside the Docker container
# if os.path.exists(model_path):
#     if torch.cuda.is_available():
#         lsc_model.load_state_dict(torch.load(model_path, map_location='cuda'))
#     else:
#         # If CUDA is not available, map the model to the CPU
#         lsc_model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
# else:
